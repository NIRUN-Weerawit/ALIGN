...
class ALIGNDataset(Dataset):
    ...
    def __init__(
        ...
        # Sidecar HDF5 handle, opened lazily on first __getitem__ if path provided.
        self._h5_dinov2: Optional[h5py.File] = None
    ...
    def _read_frames_dinov2(self, ep_idx: int, start: int, count: int) -> np.ndarray:
        """Read pre-computed DINOv2 features for a window.
        ...
        Returns:
            (count, V*257, 768) float32
        """
        if self._h5_dinov2 is None:
            if not self.dinov2_path.exists():
                raise FileNotFoundError(
                    f"DINOv2 sidecar not found: {self.dinov2_path}\n"
            ...
            self._h5_dinov2 = h5py.File(self.dinov2_path, "r")
        key = self._episode_keys[ep_idx]
        return self._h5_dinov2[f"{key}/dinov2"][start:start + count]
    ...
    def close(self):
        ...
        if hasattr(self, "_h5") and self._h5 is not None:
            self._h5.close()
            self._h5 = None
        if hasattr(self, "_h5_dinov2") and self._h5_dinov2 is not None:
            self._h5_dinov2.close()
            self._h5_dinov2 = None
