...
class ALIGNDataset(Dataset):
    ...
    def __init__(
        ...
        # Optional sidecar with pre-computed DINOv2 features. When set, the
        # `frames` field in __getitem__ returns (N, V*257, 768) float32 instead
        # of (N, V, H, W, 3) uint8. Generated by scripts/precompute_dinov2.py.
        self.dinov2_path = Path(dinov2_path) if dinov2_path else None

        if not self.h5_path.exists():
            raise FileNotFoundError(f"HDF5 file not found: {h5_path}")

        self._h5 = h5py.File(self.h5_path, "r")
        # Sidecar state for pre-computed DINOv2 features (lazy-loaded on
        # first __getitem__ when dinov2_path is set). The sidecar is a
        # directory of per-episode .npy files + index.json — see
        # scripts/precompute_dinov2.py.
        self._dinov2_index = None     # dict loaded from index.json
        self._dinov2_memmaps = None   # dict {ep_idx: np.memmap}
    ...
    def _read_frames_dinov2(self, ep_idx: int, start: int, count: int) -> np.ndarray:
        """Read pre-computed DINOv2 features for a window via memmap.

        Sidecar must be generated by scripts/precompute_dinov2.py with the
        same --cameras list as this dataset. Layout per frame:
        [cam0_patches(256), cam0_CLS(1), cam1_patches(256), cam1_CLS(1), ...]
        Total tokens per frame: V * 257 (where V = len(self.cameras)).

        Returns:
            (count, V*257, 768) float32 — a copy of the memmap slice
        """
        # Lazy index load: reads index.json once, then caches per-episode
        # memmaps as we touch them.
        if self._dinov2_index is None:
            if not self.dinov2_path.is_dir():
                raise FileNotFoundError(
                    f"DINOv2 sidecar directory not found: {self.dinov2_path}\n"
                    f"Generate with: python scripts/precompute_dinov2.py "
                    f"--data {self.h5_path} --cameras {' '.join(self.cameras)} "
                    f"--output {self.dinov2_path}"
                )
            index_path = self.dinov2_path / "index.json"
            if not index_path.exists():
                raise FileNotFoundError(
                    f"Missing index.json in {self.dinov2_path}. "
                    f"Re-run precompute_dinov2.py to regenerate."
                )
        ...
        if ep_name not in self._dinov2_index:
            raise KeyError(f"Episode {ep_name} missing from dinov2 index")

        # Open (or reuse cached) memmap for this episode
        if ep_idx not in self._dinov2_memmaps:
            npy_path = self.dinov2_path / f"{ep_name}.npy"
            if not npy_path.exists():
                raise FileNotFoundError(f"Missing {npy_path}")
            self._dinov2_memmaps[ep_idx] = np.load(
                npy_path, mmap_mode="r",
            )

        # Memmap slice returns a view; cast to a contiguous array so
        # downstream collate/torch.from_numpy don't hit weird strides.
        mm = self._dinov2_memmaps[ep_idx]
    ...
    def close(self):
        ...
        if hasattr(self, "_h5") and self._h5 is not None:
            self._h5.close()
            self._h5 = None
        # Memmaps for dinov2 sidecar release their own handles when GC'd.
        # Clear the cache so subsequent __getitem__ can re-open if needed.
        if hasattr(self, "_dinov2_memmaps") and self._dinov2_memmaps is not None:
            self._dinov2_memmaps.clear()
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # If a DINOv2 sidecar is configured, return pre-computed features
        # instead of raw uint8 frames. Shape: (count+traj_window, V*257, 768)
        # float32 — identical to model._vision_forward(B=1) output.
        if self.dinov2_path is not None:
            frames = self._read_frames_dinov2(ep_idx, start, count + self.traj_window)
        else:
...
class MultiALIGNDataset(Dataset):
    ...
    def __init__(
        ...
        traj_window: int = TRAJ_WINDOW,
        episodes_per_batch: int = 8,
        dinov2_paths: Optional[List[Optional[str]]] = None,
    ):
        if not h5_paths:
            raise ValueError("h5_paths must be a non-empty list")
        # dinov2_paths[i] is the sidecar for h5_paths[i]. None = no precompute for that file.
        if dinov2_paths is None:
            dinov2_paths = [None] * len(h5_paths)
        assert len(dinov2_paths) == len(h5_paths), \
            f"dinov2_paths length ({len(dinov2_paths)}) must match h5_paths ({len(h5_paths)})"
        self.datasets = [
            ALIGNDataset(
            ...
            for p, dp in zip(h5_paths, dinov2_paths)
        ]
        self._cumulative = [0]
...
def v4_segment_collate(batch: list, history_size: int = 20,
    ...
    padded_states = []
    padded_actions = []
    # When frames are pre-computed DINOv2 features they are float32, not uint8.
    # Detect by dtype and skip the .astype(uint8) coercion.
    frames_are_features = all(
        all_frames[b].dtype in (np.float32, np.float16, np.float64)
        for b in range(len(all_frames))
    )
    frames_out_dtype = (
        np.float32 if frames_are_features else np.uint8
    )
    for b in range(B):
        S = all_lens[b]
        # Pad frames (works for both raw uint8 and pre-computed float32 features)
        f = all_frames[b]
        if f.ndim == 4:
            # (S, H, W, 3) — single-camera raw frames
            pad_f = np.zeros((max_len, *f.shape[1:]), dtype=f.dtype)
            pad_f[:S] = f
            pad_f[S:] = f[-1:]  # replicate last frame
        else:
            # Multi-camera raw frames (S, V, H, W, 3) OR pre-computed
            # features (S, V*257, 768). Both pad the same way.
            pad_f = np.zeros((max_len, *f.shape[1:]), dtype=f.dtype)
            pad_f[:S] = f
            pad_f[S:] = f[-1:]
    ...
    return {
        "frames_segment": np.stack(padded_frames, axis=0).astype(frames_out_dtype),
